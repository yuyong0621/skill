...
# OpenClaw-first configuration paths.
...
PREFERRED_CONFIG_FILE = OPENCLAW_WORKSPACE / ".secrets" / "last30days.env"
LEGACY_CONFIG_FILE = Path.home() / ".config" / "last30days" / ".env"

# Optional overrides for testing / custom installs.
# LAST30DAYS_CONFIG_DIR="" -> disable file loading entirely
# LAST30DAYS_CONFIG_DIR="/path/to/dir" -> load /path/to/dir/.env only
# LAST30DAYS_CONFIG_FILE="/path/to/file.env" -> load that file only
_config_dir_override = os.environ.get("LAST30DAYS_CONFIG_DIR")
_config_file_override = os.environ.get("LAST30DAYS_CONFIG_FILE")

if _config_dir_override == "":
    CONFIG_FILES = []
elif _config_dir_override:
    CONFIG_FILES = [Path(_config_dir_override) / ".env"]
elif _config_file_override:
    CONFIG_FILES = [Path(_config_file_override)]
else:
    CONFIG_FILES = [PREFERRED_CONFIG_FILE, LEGACY_CONFIG_FILE]
...
def _load_merged_file_env() -> Dict[str, str]:
    """Load and merge configured .env files.

    Earlier files in CONFIG_FILES have higher priority.
    """
    merged: Dict[str, str] = {}
...
def get_config() -> Dict[str, Any]:
    """Load configuration from OpenClaw secrets + environment variables."""
    file_env = _load_merged_file_env()
...
def config_exists() -> bool:
    """Check if any configured secrets file exists."""
    return any(path.exists() for path in CONFIG_FILES if path)
...
def validate_sources(requested: str, available: str, include_web: bool = False) -> tuple[str, Optional[str]]:
    ...
    if available == 'none':
        if requested == 'auto':
            ...
            return 'web', "No API keys configured. Add keys to ~/.openclaw/workspace/.secrets/last30days.env for Reddit/X."
