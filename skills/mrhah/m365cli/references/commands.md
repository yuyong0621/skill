# m365-cli Complete Command Reference (Work/School Account)

Full reference for all `m365` CLI commands available to work/school accounts.
...
- [SharePoint Commands](#sharepoint-commands)
...
**Work account default scopes**: `Mail.ReadWrite`, `Mail.Send`, `Calendars.ReadWrite`, `MailboxSettings.Read`, `Files.ReadWrite`, `User.Read`, `User.ReadBasic.All`, `Contacts.Read`, `offline_access`

**SharePoint scope** (not included by default — requires tenant admin consent):
```bash
...
Alias: `m365 sp` = `m365 sharepoint`

> **Permission required**: `Sites.ReadWrite.All` — not included in default login scopes.
> Run `m365 login --add-scopes Sites.ReadWrite.All` and ensure tenant admin has granted consent.
...
|--------|---------|-------|
| **Path** (recommended) | `contoso.sharepoint.com:/sites/team` | Easiest to read and type |
| **Site ID** | `contoso.sharepoint.com,8bfb5166-...,ea772c4f-...` | From `sp sites --json` output |
| **URL** | `https://contoso.sharepoint.com/sites/team` | For some commands |

**Tip**: Run `m365 sp sites --json` to get the exact site identifier for any site.
...
```
m365 sp sites [options]
```

| Option | Description | Default |
|--------|-------------|---------|
| `--search <query>` | Search for sites by name | — |
| `--top <n>` | Max sites | `50` |
| `--json` | JSON output | — |

### m365 sp lists

List document libraries and lists in a site.

```
m365 sp lists <site> [options]
```

| Argument | Description |
|----------|-------------|
| `<site>` | Site identifier (path, ID, or URL format) |
...
```
m365 sp items <site> <list> [options]
```
...
```
m365 sp files <site> [path] [options]
```

| Argument | Description |
|----------|-------------|
| `<site>` | Site identifier |
| `[path]` | Path within document library (default: root) |
...
```
m365 sp download <site> <file-path> [local-path] [options]
```

| Argument | Description |
|----------|-------------|
| `<site>` | Site identifier |
| `<file-path>` | File path in document library |
| `[local-path]` | Local destination (default: original filename) |
...
```
m365 sp upload <site> <local-path> [remote-path] [options]
```
...
| `<site>` | Site identifier |
| `<local-path>` | Local file to upload |
| `[remote-path]` | Remote destination in document library |
...
```
m365 sp search <query> [options]
```

| Option | Description | Default |
|--------|-------------|---------|
| `--top <n>` | Max results | `50` |
| `--json` | JSON output | — |
...
Search users in the organization directory.
