...
const LIVE_BUY = path.join(DATA_DIR, "live_realtor_buy.json");
const LIVE_RENT = path.join(DATA_DIR, "live_realtor_rent.json");
...
function normalizeListing(raw, intent) {
  ...
  return {
    provider: "realtor",
    providerId: String(raw.propertyId || raw.url || `${addr.street}|${addr.city}|${raw.price || raw.rent || ""}`),
