...
OUTPUT_DIR = Path(
    os.environ.get("LAST30DAYS_OUTPUT_DIR", str(OPENCLAW_WORKSPACE / "data" / "last30days" / "out"))
)
...
def render_compact(report: schema.Report, limit: int = 15, missing_keys: str = "none") -> str:
    ...
    if report.mode == "web-only":
        ...
        lines.append("- Edit `~/.openclaw/workspace/.secrets/last30days.env` to add keys")
        lines.append("---")
        lines.append("")
