...
```powershell
Remove-Item -Recurse -Force "D:\clawtest\memory\lancedb"
```
Then restart gateway: `clawdbot gateway restart`
...
cron action=add job={
  ...
  "schedule": "0 4 1 * *",
  "text": "Monthly memory maintenance: 1) Wipe LanceDB path (Windows: D:/clawtest/memory/lancedb, Unix: ~/.clawdbot/memory/lancedb) 2) Parse MEMORY.md 3) Store key facts to fresh LanceDB 4) Report completion"
}
```
