...
def main():
    ...
    parser.add_argument(
        "--store",
        action="store_true",
        help="Persist findings to SQLite database (~/.openclaw/workspace/data/last30days/research.db)",
    )
    parser.add_argument(
        "--diagnose",
        action="store_true",
    ...
    if not bool(config.get("OPENAI_API_KEY")):
        source_info["reddit_skip_reason"] = "No OPENAI_API_KEY (add to ~/.openclaw/workspace/.secrets/last30days.env)"
    if not x_source:
...
def output_result(
    ...
    # Keep --emit=json machine-readable for watchlist/cron workflows.
    if web_needed and emit_mode != "json":
        print("\n" + "="*60)
